<script setup>
import { ref } from 'vue'
import { ArrowRightIcon, ChevronLeftIcon, ChevronRightIcon, DoubleArrowRightIcon } from '@radix-icons/vue'
...
<template>
  <div class="py-[18px] px-6 h-[calc(100vh-4rem)] overflow-auto">
    <div class="py-5 px-[18px] bg-background-secondary rounded-[10px] mb-[18px] border-border border-2">
      <h1 class="text-[16px] font-bold text-primary-system">About the system</h1>
      <p class="text-[14px] font-medium mt-3">This system helps pet care businesses like veterinary clinics and grooming
        services run more smoothly. It
    ...
    <div>
      <h1 class="font-medium text-[16px]">Open Revisions</h1>
      <div class="mt-2.5 flex gap-4">
        <div v-if="openRevisionsData.length === 0"
             class="flex-1 h-[225px] flex flex-col items-center justify-center border-dashed border-2 border-border-secondary rounded-[10px]">
          <p class="text-[14px] text-center">There aren't any new features being developed.</p>
          <p class="text-[14px] text-center mt-2.5">
            Go to
            <NuxtLink :to="'/revisions'" class="text-primary-system">Revisions</NuxtLink>
            to create a new one.
          </p>
        </div>
        <div v-else class="max-h-[225px] w-full flex flex-col gap-2.5">
          <div v-for="(revision, index) in openRevisionsData" :key="index"
               class="bg-background-secondary rounded-[10px] px-[9px] py-2.5">
            <div class="flex items-center">
              <div
                  class="w-[20px] h-[20px] rounded-full bg-primary-system border-border-status border-2 border-solid mr-2.5"></div>
          ...
          <div class="ml-auto">
            <NuxtLink to="/revisions" class="flex items-center gap-2">
              <p class="text-xs font-semibold">See more</p>
              <DoubleArrowRightIcon />
            </NuxtLink>
          </div>
        </div>
        <div
            class="rounded-[10px] bg-secondary-system min-w-[225px] h-[225px] px-5 py-5 relative overflow-hidden z-[1]">
          <img alt="hash" class="absolute inset-0 w-[190px] h-[190px] object-cover z-0 opacity-10 rotate-[28deg] left-[-50px] top-[75px]"
               src="~/assets/images/hash.svg"/>
